...
public class Plant
{
	...
	public PlantStage getStage()
	{
		...
		{
			if (actions.get(index).getNewStage() != null)
			{
				return actions.get(index).getNewStage();
			}
		}
	...
	public Map<PlantStage, Action> getStages()
	{
		...
		for (int index = actions.size() - 1; index >= 0; index--)
		{
			if (actions.get(index).getNewStage() != null)
			{
				stages.put(actions.get(index).getNewStage(), actions.get(index));
			}
		}
	...
	public SortedMap<PlantStage, Long> calculateStageTime()
	{
		...
		{
			if (action.getNewStage() != null)
			{
				stages.put(action.getNewStage(), action.getDate());

				if (action.getNewStage() == PlantStage.HARVESTED)
				{
